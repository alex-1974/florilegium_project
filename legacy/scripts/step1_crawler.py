...
try:
    from crawler.scoring import score_candidate as external_score_candidate  # type: ignore
except Exception:
    external_score_candidate = None

try:
    from crawler.pdf_verify import verify_pdf_candidate as external_verify_pdf_candidate  # type: ignore
except Exception:
    external_verify_pdf_candidate = None

try:
    from crawler.domain_reputation import DomainReputationStore as ExternalDomainReputationStore  # type: ignore
except Exception:
    ExternalDomainReputationStore = None

try:
    from crawler.path_semantics import analyze_path_semantics as external_analyze_path_semantics  # type: ignore
except Exception:
...
def build_config(args: argparse.Namespace) -> CrawlConfig:
    ...
    if not seed_files:
        raise SystemExit(
            ...
            "research/literature_pipeline/seeds/ "
